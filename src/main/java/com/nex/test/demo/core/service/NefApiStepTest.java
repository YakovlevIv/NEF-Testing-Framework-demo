...
public class NefApiStepTest extends AbstractTestService {

    NefApiStepTest(NefSourceServiceImpl sourceService) {
        super(sourceService);
    }

    /**
     * First step is to retrieve the base api
     * @param api base api
     */
    @Given("^targetApi is (.*)$")
    public void targetApi(String api) {
        Assert.notNull(api, "Api is null");
        Assert.isTrue(!api.isEmpty(), "Api is empty");
        this.setTargetApi(api);
    }
     ...
     * Url will be targetApi+resource
